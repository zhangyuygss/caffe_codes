function [accBoxes,accFtr,v_GT] = get_accurate_box3(PosBoxes,image_ids,v_GT,iter,image_classes)
cd('get_accurate_box/')
%
...
def_dir = '/home/zhaolong/trainCode/';
phase = 'test'; % run with phase test (so that dropout isn't applied)
net_model = [def_dir 'SO_global_vgg_deploy.prototxt'];
net_weights = [model_dir 'SO_global_iter_30000.caffemodel'];
...
if getSaliencyMap
    ...
    for idx = 1:length(imgList)
        ...
        im = prepare_img(img);
        sm = predict_sm(net1,im,size(img,1),size(img,2));
        sm = sm > 0.3;
        if ~mod(idx,100)
